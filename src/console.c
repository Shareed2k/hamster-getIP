...
void
hamster_getIP(struct mg_connection *conn, const struct mg_request_info *ri,
		void *user_data){
	struct MyProx *myprox = (struct MyProx*)user_data;
	mg_header_printf(conn,
			"HTTP/1.0 200 ok\r\n"
	...
	for (p=instance_list; *p; p += strlen(p)+1) {
		...
		sprintf_s(tmp, sizeof(tmp), "{dataParams:\"instance=%s\", leaf:true, text: \"%s\"},", instance,instance);
